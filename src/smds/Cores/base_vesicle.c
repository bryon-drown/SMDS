 ...
 *    $Id: base_vesicle.c,v 1.6 2009/09/08 21:27:28 mculbert Exp $
 *
...
typedef struct {
  ...
  double *dX, *dX2;		// dX for each species (3D, 1D)
...
int smds_core_init(PyObject *self, PyObject *args, PyObject *kwds)
{
  ...
  free(c->dX2); c->dX2 = (double*)malloc(sizeof(double)*c->numSpecies);
  ...
  if (!(c->X && c->a && c->dX && c->dX2 && c->counts && c->numFree && c->m
#ifdef CORE_BLEACH
        && c->tol && c->numBleached
#endif
#ifdef CORE_BLINK
  ...
  for(i=0; i < c->numSpecies; i++)
  {
    ...
    c->dX2[i] = sqrt(2.0 * PyFloat_AsDouble(oo) * dT) * 1e4;	// um
    if (PyErr_Occurred()) goto bail;
    Py_DECREF(oo);
...
void smds_core_create_vesicle(smds_vesicle *v, smds_core *s, int edge)
{
  ...
  // Choose Size
  do { S = Gaussian(&s->rs)*s->vSigma[type] + s->vR[type]; } while (S <= 0);
  v->r2 = S * (v->r = S);
  ...
  // Create Molecules
  num = v->numMolecs = Poisson(s->vNumMolecs[type], &s->rs);
  if (num > v->maxMolecs)
  {
    free(v->m);
    v->maxMolecs = num;
  ...
  for (i=dur; i; i--)
  {
    for (t=s->steps; t; t--)
    {
      ...
      for (vesicle=0, v=s->v; vesicle < numVesicles; vesicle++, v++)
      {
        ...
        {
          double step_x, step_y, step_z, R;
          double x = m->x, y = m->y, z = m->z;
          sp = m->species;
        
          /* Reposition */
          step_x = Normal(0, s->dX2[sp], &s->rs);
          step_y = Normal(0, s->dX2[sp], &s->rs);
          step_z = Normal(0, s->dX2[sp], &s->rs);
          
          r = x*x + y*y + z*z;
          R = (x+step_x)*(x+step_x) + (y+step_y)*(y+step_y) +
              (z+step_z)*(z+step_z);
          while (R > v->r2)
          {		// reflect
            double delta = step_x*step_x + step_y*step_y + step_z*step_z;
            double px, py, pz, rp = x*step_x + y*step_y + z*step_z;
            double ux, uy, uz, beta;
            double alpha = (sqrt(rp*rp - (r-v->r2)*delta) - rp) / delta;
            px = x + alpha*step_x;
            py = y + alpha*step_y;
            pz = z + alpha*step_z;
            rp = (x*px + y*py + z*pz); // (px*px + py*py + pz*pz) == 1
            rp = 2*rp-1;
            ux = rp*px - x;
            uy = rp*py - y;
            uz = rp*pz - z;
            beta = (1-alpha)*sqrt(delta) / sqrt(ux*ux + uy*uy + uz*uz);
            x = px;
            y = py;
            z = pz;
            r = v->r2;
