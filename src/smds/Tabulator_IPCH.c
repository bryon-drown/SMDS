 ...
 *    Integrated Pulse Counting Histogram Tabulator Helper Class
 *    $Id: Tabulator_IPCH.c,v 1.1 2008/12/06 04:27:30 mculbert Exp $
 *
 *******************************************/
...
typedef struct _smds_IPCH_Tabulator_extent extent;
struct _smds_IPCH_Tabulator_extent { 
  int num; 
  Int32 *data; 
  extent *next;
};
...
static int smds_IPCH_Tabulator_new(PyObject *self, 
					PyObject *args, PyObject *kwds);
static void smds_IPCH_Tabulator_free(smds_IPCH_Tabulator *);
// takes an smds.Task.RTR
static PyObject * smds_IPCH_Tabulator_tabulate(PyObject *self, PyObject *arg);
static PyObject * smds_IPCH_Tabulator_fetch(PyObject *self);

static PyTypeObject smds_TabulatorType = {
  PyObject_HEAD_INIT(NULL)
  0,				// ob_size
  "IPCH Tabulator",		// tp_name
  sizeof(smds_IPCH_Tabulator),	// tp_basicsize
  0,				// tp_itemsize
  (destructor)smds_IPCH_Tabulator_free,	// tp_dealloc
  0,				// tp_print
  0,				// tp_getattr
  0,				// tp_setattr
  0,				// tp_compare
  ...
  "Integrated Pulse Counting Histogram Tabulator",	// tp_doc
};

static PyObject *IPCHType = NULL;
static PyObject *RTRType = NULL;

static PyMethodDef smds_IPCH_Tabulator_methods[] = {
  {"tabulate", (PyCFunction)smds_IPCH_Tabulator_tabulate, METH_VARARGS,
   "Append more data to the IPCH.\n  "
   "Argument is of type RTR."},
  {"fetch", (PyCFunction)smds_IPCH_Tabulator_fetch, METH_NOARGS,
   "Returns the smds.Analysis.IPCH corresponding the the tabulated data."},
  {NULL}	// Sentinel
};
...
#endif
PyMODINIT_FUNC initTabulator_IPCH(void)
{
  PyObject *m;
  smds_TabulatorType.tp_new = PyType_GenericNew;
  smds_TabulatorType.tp_init = smds_IPCH_Tabulator_new;
  smds_TabulatorType.tp_methods = smds_IPCH_Tabulator_methods;
  if (PyType_Ready(&smds_TabulatorType) < 0)
    return;
  m = Py_InitModule3("Tabulator_IPCH", module_methods,
  			"Integrated Pulse Counting Histogram Tabulator");
  Py_INCREF(&smds_TabulatorType);
  PyModule_AddObject(m, "Tabulator_IPCH", (PyObject*)&smds_TabulatorType);
  import_array();

  m = PyImport_ImportModule("smds.Analysis");
  if (m)
  {
    IPCHType = PyObject_GetAttrString(m, "IPCH");
    Py_DECREF(m);
  }
...
// Num must be > 0
static extent * smds_IPCH_Tabulator_extent_new(int num)
{
  extent *r;
...
static void smds_IPCH_Tabulator_extent_free(extent *p)
{
  if (!p) return;
  smds_IPCH_Tabulator_extent_free(p->next);
  free(p->data);
  free(p);
}
...
/***
   Usual constructor:
     Begin tabulating an IPCH of the given threshold 
     from data with the given bin Width, reBinning as needed.
   Arguements: bw (ms), thr (phot/ms), bwH (ms)
***/
int smds_IPCH_Tabulator_new(PyObject *self, PyObject *args, PyObject *kwds)
{
  ...
  smds_IPCH_Tabulator_extent_free(t->head);
  t->head = smds_IPCH_Tabulator_extent_new(500);
  if (!t->head)
  {
...
/***
   Destructor
***/
void smds_IPCH_Tabulator_free(smds_IPCH_Tabulator *t)
{
  if (!t) return;
  smds_IPCH_Tabulator_extent_free(t->head);
